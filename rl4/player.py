...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        action = np.random.choice(4) if np.random.random() < epsilon else np.nanargmax(Q[state])
        # ADD YOUR CODE SNIPPET BETWEEN EX 4.1
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max and diff>self.threshold:
            ...
            while not end_episode:
                ...
                max_future_reward = np.nanmax(Q[s_next])
                new_q_value = Q[s_current][action] + lr * (R + (discount * max_future_reward) - Q[s_current][action])
                Q[s_current][action] = new_q_value
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
